...
#[test]
fn decode_accepts_legacy_2930_1559_4844_7702() {
    let signer = test_signer();
    ...
    let eip4844 = decode_eth_raw_tx(&sign_encoded(tx_4844(CHAIN_ID, 3), &signer)).unwrap();
    assert_eq!(eip4844.caller, expected);
    assert_eq!(eip4844.tx_type, 3);
    assert_eq!(eip4844.blob_hashes.len(), 1);
    assert_eq!(eip4844.max_fee_per_blob_gas, 4);

    let eip7702 = decode_eth_raw_tx(&sign_encoded(tx_7702(CHAIN_ID, 4, &signer), &signer)).unwrap();
    assert_eq!(eip7702.caller, expected);
    assert_eq!(eip7702.tx_type, 4);
    assert_eq!(eip7702.authorization_list.len(), 1);
}
